...
## M1-T01R2 repair evidence

PR #6 now contains the M1-T01R2 closure evidence: unified mapped exclusions cover fixed synonyms, bridge terms, deterministic wide terms, and fake-provider expansions; core-term conflicts reject with `CONTRADICTORY_INTENT`; `candidate_synonyms` become restricted `LLM_FAKE` QueryExpansion records; NFKC evidence matching and unique 12-query plans are enforced; and live M0 completion regression is rejected. M1 remains `IN_PROGRESS 1/4`; M1-T02 is explicitly not started and outside this repair scope.
